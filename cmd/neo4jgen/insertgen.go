...
func main() {
	...
	for _, id := range ids {
		for i := 0; i < qtd; i++ {
			...
			_, errCreate := f.WriteString(fmt.Sprintf("CREATE (e:Vertex {id: '%s', type: 'E'});\n", newId))
			if errCreate != nil {
			...
			_, errAdd := f.WriteString(fmt.Sprintf("MATCH (d:Vertex), (e:Vertex) WHERE d.id = '%s' AND e.id = '%s' CREATE (d)-[:has]->(e);\n", id, newId))
			if errAdd != nil {
				fmt.Println(errAdd)
				f.Close()
				return
